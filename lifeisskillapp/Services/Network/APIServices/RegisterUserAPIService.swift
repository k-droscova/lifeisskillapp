...
protocol RegisterUserAPIServicing {
    ...
    //func completeRegistration(credentials: FullRegistrationCredentials, location: UserLocation?) async throws -> APIResponse<RegistrationResponse>
}
...
public final class RegisterUserAPIService: BaseClass, RegisterUserAPIServicing {
    ...
    /*func completeRegistration(credentials: FullRegistrationCredentials, location: UserLocation?) async throws -> APIResponse<RegistrationResponse> {
        ...
        let task = ApiTask.completeRegistration(credentials: credentials, location: location)
        let data = try task.encodeParams()
